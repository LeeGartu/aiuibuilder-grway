...
lv_result_t lv_ffmpeg_player_set_src(lv_obj_t * obj, const char * path)
{
	...
	lv_image_cache_drop(&player->imgdsc);
                                   ...
                                   lv_image_header_t * header)
{
    ...
    if(avformat_open_input(&fmt_ctx, filepath + 2, NULL, NULL) < 0) {
        LV_LOG_ERROR("Could not open source file %s", filepath + 2);
        goto failed;
    }
...
struct ffmpeg_context_s * ffmpeg_open_file(const char * path)
{
    ...
    if(avformat_open_input(&(ffmpeg_ctx->fmt_ctx), path + 2, NULL, NULL) < 0) {
        LV_LOG_ERROR("Could not open source file %s", path + 2);
        goto failed;
    }
